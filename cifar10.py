...
tf.app.flags.DEFINE_string('data_dir', 'C:\\Users\\SOL\\PycharmProjects\\untitled1\\image_sol',
...
def distorted_inputs():
  ...
  data_dir = os.path.join(FLAGS.data_dir, 'train_data')
...
def inputs(eval_data):
  ...
  if not FLAGS.data_dir:
    raise ValueError('Please supply a data_dir')
  data_dir = os.path.join(FLAGS.data_dir, 'test_data')
...
def inference(images):
  ...
  with tf.variable_scope('local5') as scope:
    ...
    # dropout 추가
    drop_local5 = tf.nn.dropout(local5, keep_prob=0.5)
  ...
  with tf.variable_scope('local6') as scope:
    ...
    local6 = tf.nn.relu(tf.matmul(drop_local5, weights) + biases, name=scope.name)
    _activation_summary(local6)
    #dropout 추가해주었다.
    drop_local6 = tf.nn.dropout(local6, keep_prob=0.5)
  ...
  with tf.variable_scope('softmax_linear') as scope:
    ...
    softmax_linear = tf.add(tf.matmul(drop_local6, weights), biases, name=scope.name)
    _activation_summary(softmax_linear)
    # dropout 추가해주었다.
    drop_softmax_linear = tf.nn.dropout(softmax_linear, keep_prob=0.5)
  return drop_softmax_linear
